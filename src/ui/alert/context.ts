...
export function useAlert() {
  const call = (opts: ShowAlertOptions) => {
    try {
      resolveShow()(opts)
    } catch {
      switch (opts.variant) {
        case 'error':
          console.error('Alert:', opts.title, opts.message)
          return
        case 'info':
          console.info('Alert:', opts.title, opts.message)
          return
        default:
          console.log('Alert:', opts.title, opts.message)
          return
      }
    }
  }
