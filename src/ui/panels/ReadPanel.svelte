<script lang="ts">
  ...
  import { formatMs } from '@/sys/generic/helpers'
  ...
  import WordTable from '@/ui/generics/WordTable.svelte'
  ...
  let addressResolved = $derived(names.get(settings.queryTemplate.address))
        ...
        <Alert.Root variant={errorMsg ? 'destructive' : 'default'} class="bg-transparent">
          {#if lastRes}
            <Alert.Title>
              ...
              {lastRes.data.length} registers from {HEX(lastRes.startAddress, 4)} to {HEX(
                lastRes.startAddress + lastRes.data.length - 1,
                4,
              )}
            </Alert.Title>
