<script lang="ts">
  ...
  function onKeydown(e: KeyboardEvent) {
    ...
    if (e.key === 'Backspace' && s.trim() === '') {
      if (value.length) {
        ...
        queueMicrotask(() => {
          inputEl?.focus();
          const len = s.length; inputEl?.setSelectionRange(len, len);
        });
        e.preventDefault();
      }
    }
  ...
  function onPaste(e: ClipboardEvent) {
    ...
    for (const tok of toks) {
      ...
      if (b === null) { s = tok; inputEl?.focus(); return; }
      value = [...value, b];
    }
    s = '';
  }
...
<label class={`flex items-center gap-2 rounded-md border bg-background px-2 py-1 text-sm shadow-sm focus-within:ring-2 focus-within:ring-ring ${className}`} aria-label="Boolean values">
  <input
    bind:this={inputEl}
  ...
  {#if buttons}
    <span class="inline-flex gap-1">
      <button type="button" class="h-7 w-7 rounded-md border text-sm font-mono hover:bg-foreground/5" onclick={() => add(false)}>0</button>
      <button type="button" class="h-7 w-7 rounded-md border text-sm font-mono hover:bg-foreground/5" onclick={() => add(true)}>1</button>
    </span>
  {/if}
</label>
