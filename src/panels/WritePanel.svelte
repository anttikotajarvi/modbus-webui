<script lang="ts">
  ...
  const writeToClient = getContext<(query: WriteQuery) => Promise<WriteResponse>>('writeToClient')
    ...
    <Card.Header class="flex items-center justify-between gap-3">
      ...
      <form class="flex flex-wrap items-end gap-3" onsubmit={writeData}>
        <div class="flex max-w-sm flex-col gap-1.5">
          <Label for="address" class="text-muted-foreground">Start Address</Label>
        ...
        <div class="flex max-w-sm flex-col gap-1.5">
          ...
          {#if type === 'write_coils'}
            <BinaryArrayInput bind:value={bitValues} class="h-9 w-40" inputClass="w-full" />
          {:else if type === 'write_registers'}
            <HexArrayInput
      ...
      <Card.Footer class="flex flex-col gap-4 items-start">
        ...
        <div class="flex items-end gap-3 pt-4">
          <div class="flex flex-col gap-1.5">
            <Label for="shortcutName" class="text-muted-foreground">Shortcut name</Label>
            <TagInput
              id="shortcutName"
