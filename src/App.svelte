<script lang="ts">
  ...
  const readFromClient = async (query: ReadQuery) => {
    ...
    const methodMap = {
      read_input_registers: (c: ModbusRTU, a: number, q: number) =>
        c.readInputRegisters(a, q),
      read_holding_registers: (c: ModbusRTU, a: number, q: number) =>
        c.readHoldingRegisters(a, q),
      read_coils: (c: ModbusRTU, a: number, q: number) => c.readCoils(a, q),
      read_discrete_inputs: (c: ModbusRTU, a: number, q: number) =>
        c.readDiscreteInputs(a, q),
    };

    const method = methodMap[query.type];

    const res = await method(client, query.address, query.quantity).catch(
      (err: any) => {
  ...
  function Connect() {
    ...
    const { deviceId, options } =
      lib.profiles[activeProfileId ?? SCRATCH_ID].connectionSettings;
  ...
  let activeProfileId: Readonly<TAG> = $derived(
    lib.activeProfileTag ?? SCRATCH_ID
  );
  ...
  let currentNTS: Readonly<NameTableSet> = $derived.by(() =>
    currentProfile.nameTableSetId
      ? (lib.nameTables[currentProfile.nameTableSetId] ??
        createEmptyNameTableSet())
      : createEmptyNameTableSet()
  );
...
<div class="flex min-h-dvh flex-col">
  ...
  <main class="flex-1 p-4 min-w-0">
    ...
    <div
      class="grid gap-6 min-w-0 min-[1480px]:grid-cols-[minmax(0,1fr)_clamp(700px,28vw,900px)]"
    >
      ...
      <aside
        class="min-w-0 w-full space-y-4 self-start min-[1480px]:sticky min-[1480px]:top-4"
      >
  ...
  <NameTableSetModal
    ...
    readNTS={(id: TAG) =>
      $state.snapshot(lib.nameTables[id]) ?? createEmptyNameTableSet()}
