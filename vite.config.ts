...
function shouldIgnoreSvelteWarning(warning: { code?: string; message?: string }) {
  if (warning.code === 'custom_element_props_identifier') return true

  if (warning.code === 'state_referenced_locally') {
    const name = warning.message?.match(/`([A-Za-z_$][\w$]*)`/)?.[1]

    if ( name == "lib") return false;

    return name?.endsWith('Ref') === true
  }
...
export default defineConfig({
  ...
  plugins: [
    ...
    createHtmlTokenReplace({
      // This is mainly used for the footer.
    ...
    transformIndexHtml(html): IndexHtmlTransformResult {
      ...
      const tags = [
        ...
        // Core meta
        {
          ...
          attrs: {
            name: 'description',
            content: 'Single-file Modbus Web UI with profiles, name tables, and shortcuts',
          },
        },
...
function createHtmlTokenReplace(
  ...
  const pattern =
    keys.length > 0
      ? new RegExp(`${esc(delimStart)}(${keys.map(esc).join('|')})${esc(delimEnd)}`, 'g')
      : null
